...
export default function ProductionPage() {
  // 認証状態を取得
  const { user, loading, isProduction, logout } = useAuth();
  ...
  // 認証チェック - 制作会社でない場合はログインページにリダイレクト
  useEffect(() => {
    if (!loading) {
      if (!user) {
        router.push('/login');
      } else if (!isProduction) {
        // ユーザーはログインしているが制作会社権限がない場合
        alert('制作会社権限がありません');
        router.push('/');
      }
    }
  }, [loading, user, isProduction, router]);
    ...
    <div className="min-h-screen bg-gray-100 dark:bg-gray-900">
      ...
      <header className="bg-white dark:bg-gray-800 shadow-md">
        <div className="container mx-auto px-4 py-4 flex justify-between items-center">
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">制作会社ダッシュボード</h1>
          <div className="flex items-center space-x-4">
            <span className="text-sm text-gray-600 dark:text-gray-300">
              {user?.user_metadata?.name || ''}さん（制作会社）
            </span>
            <Button 
              variant="outline" 
              size="sm"
